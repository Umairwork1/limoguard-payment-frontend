...
interface Props {
  onMethodSelect: (methodId: number, invoiceValue: number, currency: CurrencyIso) => void;
}

export default function DirectInitiate({ onMethodSelect }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const res = await api.directInitiate({ invoiceAmount: amount, currencyIso: currency });
  ...
  return (
    <div>
      ...
      <p className="section-desc">
        Calls <code>POST /api/direct/initiate</code> to retrieve available direct payment methods
        and their service charges for the given amount.
      </p>
                ...
                <tr key={m.PaymentMethodId}>
                  ...
                  <td>
                    <button
                      ...
                      onClick={() => onMethodSelect(m.PaymentMethodId, amount, currency)}
                    >
                      Use
                    </button>
                  </td>
